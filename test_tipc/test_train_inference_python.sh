...
exp_dir_key=$(func_parser_key "${lines[6]}")
exp_dir_value=$(func_parser_params "${lines[6]}" "${MODE}")
...
data_path_value=$(func_parser_params "${lines[10]}" "${MODE}")
    ...
    for use_gpu in ${use_gpu_list[*]}; do
        ...
        if [ ${use_gpu} = "False" ] || [ ${use_gpu} = "cpu" ]; then
            for batch_size in ${batch_size_list[*]}; do
                ...
                set_exp_dir=$(func_set_params "${exp_dir_key}" "${exp_dir_value}")
                set_image_path=$(func_set_params "${image_dir_key}" "${infer_img_dir}")
                set_save_infer_key=$(func_set_params "${save_infer_key}" "${save_infer_path}")

                command="${_python} ${_script} ${use_gpu_key}=${use_gpu} ${set_save_infer_key} ${set_batchsize} ${set_infer_data} ${set_benchmark} ${set_image_path} > ${_save_log_path} 2>&1 "
                eval $command
                #echo $command
...
if [ ${MODE} = "whole_infer" ]; then
    ...
    for gpu in ${gpu_list[*]}; do
        ...
        for trainer in ${trainer_list[*]}; do
            ...
            set_exp_dir=$(func_set_params "${exp_dir_key}" "${exp_dir_value}")
            set_data_dir=$(func_set_params "${data_path_key}" "${data_path_value}")
            ...
            if [ ${#gpu} -le 2 ];then  # train with single gpu
                cmd="${python} ${run_train} ${set_exp_dir} ${set_epoch} ${set_batchsize} ${set_data_dir}"
            elif [ ${#ips} -le 15 ];then  # train with multi-gpu
                cmd="${python} -m paddle.distributed.launch --gpus=${gpu} ${run_train} ${set_exp_dir} ${set_epoch}  ${set_batchsize} ${set_data_dir}"
            else     # train with multi-machine
                cmd="${python} -m paddle.distributed.launch --ips=${ips} --gpus=${gpu} ${run_train} ${set_exp_dir} ${set_epoch}  ${set_batchsize} ${set_data_dir}"
            fi
            ...
            if [ ${eval_py} != "null" ]; then
                eval ${env}
                #echo $env
                eval_cmd="${python} ${eval_py}  ${set_data_dir}  ${set_exp_dir} " 
                eval $eval_cmd
                #echo $eval_cmd
            ...
            if [ ${run_export} != "null" ]; then 
                ...
                export_cmd="${python} ${run_export} ${set_exp_dir} ${set_save_infer_key}"
                eval $export_cmd
                #echo $export_cmd
