...
def nf_fast_flow(input_chw, conv3x3_only, hidden_ratio, flow_steps, clamp=2.0):
    ...
    for i in range(flow_steps):
        if i % step == (step - 1) and not conv3x3_only:
            kernel_size = 1
        else:
            kernel_size = 3
        nodes.append(
    ...
    def forward(self, x):
        ...
        if not self.training:
            ...
            for output in outputs:
                log_prob = -paddle.mean(output**2, axis=1, keepdim=True) * 0.5
                prob = paddle.exp(log_prob)
